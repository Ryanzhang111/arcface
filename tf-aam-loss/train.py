...
def main(_):
    ...
    with tf.device('/cpu:0'), tf.name_scope('input') as scope:
        ...
        train_dataset = (tf.data.Dataset
            ...
            .shuffle(100)
            .flat_map(lambda f: tf.data.TFRecordDataset(f))
            .shuffle(10000)
    ...
    if FLAGS.restore:
        ...
        if not FLAGS.load_fc:
            vars_to_restore = [v for v in vars_to_restore if not v.name.startswith('AAM')]
    ...
    for e in range(FLAGS.start_epoch, FLAGS.end_epoch):
        ...
        try:
            ...
            while True:
                sess.run(
                    [metrics_update_op], {is_training: False})
        except tf.errors.OutOfRangeError:
            pass
