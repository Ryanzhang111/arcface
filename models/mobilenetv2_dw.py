...
def mobile_block(inputs, num_output, kernel_size, stride=1, t=6, scope=None):
    with tf.variable_scope(scope, default_name='mobile_block'):
        ...
        outputs = slim.conv2d(
            ...
            normalizer_params={'param_initializers': {'gamma': tf.constant_initializer(0.01)}},
            scope='point_conv')

        return outputs + shortcut


def build_net(images, n_class=None, is_training=True, reuse=False, alpha=1.25, scope='mobile_id'):
    ...
    with tf.variable_scope(scope, reuse=reuse) as scope:
        ...
        with slim.arg_scope([slim.batch_norm], is_training=is_training):
            ...
            net = mobile_block(net, int(16*alpha), 3, stride=1, t=1, scope='mblock1_1')
            net = mobile_block(net, int(16*alpha), 3, stride=1, scope='mblock1_2')

            net = mobile_block(net, int(24*alpha), 3, stride=2, scope='mblock2_1')
            net = mobile_block(net, int(24*alpha), 3, stride=1, scope='mblock2_2')
            net = mobile_block(net, int(24*alpha), 3, stride=1, scope='mblock2_3')
            ...
            net = mobile_block(net, int(32*alpha), 3, stride=1, scope='mblock3_4')
